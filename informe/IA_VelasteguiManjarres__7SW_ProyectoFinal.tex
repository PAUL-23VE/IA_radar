...
Una pregunta central es si el sistema funciona únicamente sobre los datos con los que
se entrenó (placas, caracteres y videos específicos) o si \emph{generaliza} a casos
nuevos. La evidencia y el diseño indican que el sistema generaliza, con condiciones:

\begin{itemize}
    \item \textbf{Detección y seguimiento de vehículos.} El modelo \texttt{yolo11n}
    está preentrenado sobre COCO (un conjunto general de 80 categorías que incluye
    automóvil, motocicleta, bus y camión). No se entrenó con nuestros videos, por lo
    que detecta y rastrea vehículos en cualquier escena o cámara.
    \item \textbf{Detección de placa.} El detector se entrenó con un conjunto
    internacional de $\sim$101\,866 imágenes de placas \cite{ds_placas}, no con nuestro
    video. En pruebas a distintas escalas localizó la placa de forma consistente, lo
    que confirma que aprende la apariencia general de una placa y no casos memorizados.
    \item \textbf{Reconocimiento de caracteres.} La U-Net y la CNN se entrenaron con
    placas de Reino Unido, Brasil e India y recortes ecuatorianos
    \cite{ds_uk,ds_brasil,ds_seg,ds_ec}. El hecho de que lean placas ecuatorianas de
    campus ---ausentes del entrenamiento--- demuestra generalización; la votación
    temporal entre fotogramas corrige los errores puntuales de un solo cuadro.
\end{itemize}

En consecuencia, el sistema \emph{no} está limitado al video usado en las pruebas:
opera sobre cualquier cámara, vehículo y placa. No obstante, su alcance y desempeño
están acotados por:

\begin{itemize}
    \item \textbf{Formato ecuatoriano.} La validación final acepta solo la estructura
    de tres letras y tres o cuatro dígitos; placas de otros países se descartan de
    forma intencional, pues el sistema está dirigido al contexto nacional.
    \item \textbf{Condiciones de captura.} La lectura es confiable cuando la placa se
    observa con suficientes píxeles (vehículo cercano), de forma razonablemente frontal
    y sin desenfoque extremo, suciedad u oclusión; placas muy lejanas o muy borrosas
    reducen la exactitud, según el límite de muestreo espacial ya descrito.
    \item \textbf{Calibración de la velocidad.} La exactitud de la velocidad depende de
    una distancia entre líneas correctamente medida, su posición acorde a la escena y un
    fps efectivo suficiente.
    \item \textbf{Confusiones residuales.} Persisten confusiones entre caracteres
    visualmente similares (O/0, I/1), mitigadas ---no eliminadas--- por la votación.
\end{itemize}

En síntesis, para un despliegue nuevo basta con apuntar la cámara, calibrar las dos
líneas a la distancia real y asegurar que las placas se vean legibles; no se requiere
