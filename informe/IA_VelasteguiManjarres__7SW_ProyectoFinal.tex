...
La Tabla~\ref{tab:dependencias} detalla cada dependencia, su función y el uso
concreto que se le dio dentro del sistema. Todas son de código abierto.
...
\begin{table}[H]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{2.6cm} >{\raggedright\arraybackslash}p{4.2cm} >{\raggedright\arraybackslash}p{7.0cm}}
        \toprule
        \textbf{Dependencia} & \textbf{Qué hace} & \textbf{Cómo se usó en el sistema} \\
        \midrule
        \texttt{ultralytics} & Implementación de modelos YOLO y del seguimiento
        multi-objeto (ByteTrack). & Carga \texttt{yolo11n} (COCO) para detectar y
        \emph{rastrear} todos los vehículos del cuadro, y el modelo de placa
        (\emph{License\_Plate}) para localizar la región de la placa en cada vehículo. \\

        \texttt{onnxruntime} & Motor de inferencia de modelos en formato ONNX,
        independiente del framework de entrenamiento. & Ejecuta en CPU la U-Net de
        segmentación de caracteres y la CNN de clasificación, convertidas desde Keras.
        Permite usar el OCR en Python~3.14 sin TensorFlow. \\

        \texttt{torch} & Backend de cómputo en GPU de los modelos YOLO. & Acelera en
        CUDA la detección y el seguimiento de vehículos/placas; la GPU queda dedicada
        a YOLO mientras el OCR corre en CPU vía ONNX. \\

        \texttt{opencv-python} & Procesamiento de imagen y video clásico. & Captura de
        cámara/archivo, recorte y escalado de placas, enderezado por Hough,
        binarización y componentes conexos en el postproceso, y dibujo del HUD
        (líneas, cajas, etiquetas) sobre el video. \\

        \texttt{numpy} & Cálculo numérico vectorial sobre matrices. & Representa
        imágenes y máscaras, arma los \emph{batches} de entrada a los modelos ONNX y
        realiza la votación y los cálculos del sistema difuso. \\

        \texttt{scikit-fuzzy} & Funciones de pertenencia difusa (triangular,
        trapezoidal) e interpolación. & Define las pertenencias de velocidad y los
        sub-niveles de severidad, base de la clasificación Mamdani y de la sanción
        Takagi--Sugeno. \\

        \texttt{fastapi} / \texttt{uvicorn} & Framework y servidor web asíncrono. &
        Exponen la API REST (iniciar/detener, configurar fuente y distancia),
        transmiten el video procesado (MJPEG) y emiten los eventos al frontend. \\

        \texttt{matplotlib} & Generación de figuras. & Renderiza el razonamiento difuso
        (pertenencias de velocidad y curva de sanción) que se muestra en el modal del
        evento y se adjunta en el correo. \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    se calcula la sanción mediante un sistema de inferencia Takagi--Sugeno de orden
    cero \cite{sugeno}. En lugar de una única membresía de entrada, se definen cinco
    sub-niveles de severidad ---leve, moderada, alta, grave y extrema--- que cubren el
    rango de velocidades 20--40~km/h con solapamiento al 50\,\%. Cada sub-nivel cuenta
    con un consecuente \emph{singleton} (valor crisp) que representa las horas de
    indisponibilidad: 5, 18, 34, 52 y 70 horas respectivamente. La salida se obtiene
    como promedio ponderado de estos singletons según la activación de cada sub-nivel:
    \begin{equation}
        H(v) = \frac{\sum_{i=1}^{5} \mu_i(v)\, c_i}{\sum_{i=1}^{5} \mu_i(v)},
        \label{eq:sugeno}
    \end{equation}
    donde $\mu_i(v)$ es el grado de pertenencia de la velocidad $v$ al sub-nivel $i$,
    y $c_i$ es su consecuente singleton en horas. Esta formulación garantiza una salida
    \emph{suave, monótona y proporcional}: un aumento en la velocidad produce siempre
    un aumento proporcional en la sanción, sin mesetas ni discontinuidades.
\end{enumerate}

\paragraph{Justificación de Takagi--Sugeno vs.~Mamdani.} La elección de Takagi--Sugeno
sobre el defuzzificador de centroide del método Mamdani es fundamental. En un enfoque
Mamdani de clasificación de sanción basado en el máximo de varias pertenencias
triangulares solapadas, el centroide de la agregación produce una curva de salida con
mesetas locales y micro-descensos no monótonos cerca de los bordes entre conjuntos
---efecto denominado ``serrucho''---. Esta no-monotonicidad es inaceptable en una
aplicación legal: significa que a veces un vehículo a 21~km/h recibe mayor sanción que
uno a 22~km/h, violando el principio de proporcionalidad requerido por regulaciones de
tránsito que especifican castigos graduales según el exceso de velocidad
\cite{reglamento}. El promedio ponderado de singletons~\eqref{eq:sugeno} evita este
problema: la combinación convexa de valores crecientes ($c_1 < c_2 < \cdots < c_5$)
asegura monotonicidad estricta en toda la curva de sanción. Por tanto, Sugeno es no
solo técnicamente preferible sino también jurídicamente más defensible, pues refleja
una sanción que escala suavemente y sin contradicciones con el exceso de velocidad.
...
El sistema exhibe una capacidad significativa de generalización más allá de los datos y
escenas utilizados durante el entrenamiento. Ello se debe a la arquitectura modular y
a la elección de modelos preentrenados de alcance general. La demostración de
generalización se fundamenta en los siguientes factores:

\begin{itemize}
    \item \textbf{Detección y seguimiento de vehículos.} El modelo base YOLO11n proviene
    del entrenamiento en COCO \cite{yolo11}, un conjunto de 80 categorías de alcance
    internacional que incluye automóvil, motocicleta, bus y camión. Puesto que dicho
    modelo no fue reentrenado con los videos del proyecto, es capaz de detectar y
    rastrear vehículos en escenas y cámaras diversas, demostrando que ha aprendido
    patrones de forma de vehículos más allá del dominio del entrenamiento.
    \item \textbf{Detección de placa.} El detector de placas fue entrenado sobre un
    conjunto internacional de aproximadamente 101\,866 imágenes \cite{ds_placas},
    ajeno a los videos del campus. Pruebas a diferentes escalas y condiciones
    demostraron que el detector localiza la placa de manera consistente, indicando que
    ha aprendido la estructura geométrica general de una placa vehicular, en lugar de
    memorizar casos específicos.
    \item \textbf{Reconocimiento de caracteres.} La red U-Net y el clasificador CNN
    fueron entrenados sobre datos de placas de Reino Unido, Brasil, India y Ecuador
    \cite{ds_uk,ds_brasil,ds_seg,ds_ec}, procedentes de múltiples fuentes y contextos.
    El hecho de que estos modelos lean correctamente placas ecuatorianas del campus
    ---datos ausentes completamente del conjunto de entrenamiento--- constituye evidencia
    directa de generalización; la votación temporal entre fotogramas absorbe además los
    errores ocasionales de fotogramas individuales, mejorando la confiabilidad.
\end{itemize}

En consecuencia, el sistema no está confinado al video de prueba: es capaz de operar
sobre cámaras, vehículos y placas nuevos. Su capacidad operativa y exactitud están
limitadas, sin embargo, por factores inherentes al diseño y al dominio:

\begin{itemize}
    \item \textbf{Restricción de formato.} La validación final impone la estructura de
    tres caracteres alfabéticos seguidos de tres o cuatro dígitos, acorde al estándar
    ecuatoriano. Placas de otros países se rechazan de forma deliberada, en concordancia
    con el enfoque específico del sistema.
    \item \textbf{Dependencia de condiciones ópticas.} La lectura confiable requiere que
    la placa se observe con resolución suficiente (vehículo en proximidad), ángulo de
    observación aproximadamente frontal, y ausencia de desenfoque extremo, suciedad u
    oclusión. Placas lejanas o severamente desenfocadas experimentan degradación de
    exactitud, conforme a los límites de muestreo espacial descritos en metodología.
    \item \textbf{Calibración de distancia en velocidad.} La exactitud de la medición de
    velocidad depende críticamente de que la distancia entre líneas de medición sea
    correctamente calibrada a la geometría real de la escena, que su posicionamiento sea
    acorde a la topografía de la zona, y que la frecuencia de fotogramas sea suficiente
    para capturar transiciones.
    \item \textbf{Confusiones residuales de similitud.} El clasificador exhibe confusión
    ocasional entre caracteres morfológicamente similares (p.~ej.\ O/0, I/1), efecto
    que se mitiga ---pero no se elimina--- mediante la votación temporal.
\end{itemize}

Por tanto, para despliegues futuros es suficiente orientar la cámara hacia la zona de
medición, calibrar las dos líneas virtuales a la distancia real medida en el terreno, y
asegurar que las placas sean observadas con claridad óptica. No es requerido el
