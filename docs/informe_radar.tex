...
\subsection{Conceptos Nucleares de Entrenamiento}
\begin{itemize}
    \item \textbf{Backpropagation y la Regla de la Cadena:} El modelo hace una predicci\'on y calcula el error (P\'erdida). \emph{Backpropagation} aplica c\'alculo diferencial (regla de la cadena de derivadas parciales) desde la salida hacia la entrada para calcular c\'omo cada peso de la red contribuy\'o al error, y los ajusta (Descenso del Gradiente).
    \item \textbf{Funciones de Activaci\'on (ReLU vs Sigmoide):} Las redes necesitan no-linealidad. La \emph{Sigmoide} (curva en S) satura en 0 y 1, causando que sus derivadas sean casi cero en los extremos (el mortal \textbf{Vanishing Gradient Problem}), impidiendo que las redes profundas aprendan. Usamos \textbf{ReLU} ($f(x) = \max(0, x)$) porque su derivada es 1 si $x>0$, permitiendo que el gradiente fluya intacto hacia las capas iniciales.
    \item \textbf{Batch Normalization:} En redes profundas, la distribuci\'on de las entradas a cada capa cambia constantemente mientras se actualizan los pesos (\emph{Internal Covariate Shift}). \emph{BatchNorm} normaliza (media 0, varianza 1) las activaciones por cada \emph{batch}, estabilizando el entrenamiento y permitiendo Tasas de Aprendizaje (LR) m\'as altas.
    \item \textbf{Dropout:} Es una regularizaci\'on extrema. Durante el entrenamiento, apagamos aleatoriamente (ej. 50\%) algunas neuronas. Esto obliga a la red a \textbf{no depender} de una sola neurona o caracter\'istica para predecir, haci\'endola robusta y mitigando el \emph{Overfitting}.
\end{itemize}
...
Si una imagen tiene $48 \times 48 = 2304$ p\'ixeles, una primera capa densa oculta con 1000 neuronas requerir\'ia $2.3$ Millones de pesos. Adem\'as, destruir\'ia la estructura topol\'ogica 2D de la imagen (un p\'ixel vecino al otro).
Las \textbf{Convoluciones} aplican un \emph{filtro/kernel} peque\~no (ej. $3 \times 3$) que se desliza por la imagen. Esto tiene dos superpoderes:
1. \textbf{Par\'ametros Compartidos:} El mismo filtro ($3 \times 3 = 9$ pesos) busca un patr\'on en toda la imagen.
2. \textbf{Invarianza Espacial:} Si el filtro detecta un barde vertical en la esquina, podr\'a detectarlo igual en el centro.
...
Como YOLO predice sobre una grilla, a menudo arroja 5 cajas superpuestas para el mismo veh\'iculo. \textbf{NMS} soluciona esto:
1. Toma la caja con mayor confianza.
2. Descarta todas las dem\'as cajas que tengan un IoU (Intersection over Union) muy alto con la caja ganadora (ej. $>0.45$).
3. Repite para el siguiente objeto.
